...
impl Backend {
  ...
  pub fn render(&self, instrument: &Instrument) -> Result<()> {
    ...
    let peaks_marker_distance = env::var("MARKERS_DISTANCE").unwrap().parse::<f64>().unwrap();
    let output_file = [&env::var("OUTPUT_FOLDER").unwrap(), instrument.ticker(), ".png"].concat();
    ...
    for x in local_minima.iter() {
      chart
        .draw_series(LineSeries::new(
          (0..).zip(local_minima.iter()).map(|(_k, highs)| {
            let idx = highs.0;
            let value = highs.1;
    ...
    rsi_pannel
      .draw_series(LineSeries::new(
        (0..).zip(data.iter()).map(|(id, candle)| (candle.date(), rsi_a[id])),
        &RED,
      ))
      .unwrap();
    ...
    stoch_pannel
      .draw_series(LineSeries::new(
        (0..).zip(data.iter()).map(|(id, candle)| (candle.date(), stoch_a[id])),
        &BLUE,
      ))
      .unwrap();
    ...
    stoch_pannel
      .draw_series(LineSeries::new(
        (0..).zip(data.iter()).map(|(id, candle)| (candle.date(), stoch_b[id])),
        &RED,
      ))
      .unwrap();
    ...
    stoch_pannel
      .draw_series(LineSeries::new(
        (0..).zip(data.iter()).map(|(id, candle)| (candle.date(), macd_a[id])),
        &BLUE,
      ))
      .unwrap();
    ...
    stoch_pannel
      .draw_series(LineSeries::new(
        (0..).zip(data.iter()).map(|(id, candle)| (candle.date(), macd_b[id])),
        &RED,
      ))
      .unwrap();
